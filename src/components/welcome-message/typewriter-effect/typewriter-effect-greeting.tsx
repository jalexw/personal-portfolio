...
export function TypewriterEffectGreeting({
  message,
}: {
  message: string[];
}): ReactElement {
  ...
  useEffect(() => {
      ...
      const controls = animate<number>(count, 0, {
        ...
        onComplete(): void {
          if (count.get() < 1) {
            if (process.env.NODE_ENV === "development") {
              console.log(
                '[WelcomeMessage] Firing "typewriter_effect_exit_complete" event',
              );
            }
            experience.dispatch({
            ...
            if (process.env.NODE_ENV === "development") {
              console.log(
                '[WelcomeMessage] Firing "placeholder_exit_complete" event',
              );
            }
            experience.dispatch({
    ...
    return;
  }, [animate, message, count, isPresent, safeToRemove, experience]);
